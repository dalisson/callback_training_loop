...
class ParamScheduler(Callback):
    ...
    def begin_fit(self):
        ...
        for sched_name in self.run.scheduler_states.keys():
            if self.name == sched_name:
                self.train_iter = self.run.scheduler_states['train_iter']
...
def one_cycle_scheduler(lrs, n_param_groups, max_lr, min_mom=0.85, mom=0.95,
                        divs=None):
    ...
    for base_lr, m_lr in zip(lrs, max_lr):
        func = combine_scheds(divs, [sched_cos(base_lr, m_lr),
                                     sched_cos(m_lr, base_lr*1e-1)])
    ...
    for mom in base_moms:
        func = combine_scheds(divs, [sched_cos(mom, min_mom),
                                     sched_cos(min_mom, mom)])
        sched_funcs.append(func)
