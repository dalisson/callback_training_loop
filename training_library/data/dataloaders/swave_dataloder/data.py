...
__all__=["Trainset", "Validset", "build_swave_dataloader"]
...
class Trainset:
    ...
    def __getitem__(self, index):
        ...
        return self.mix_set[index], torch.stack(tgt_sig), torch.LongTensor([mix_sig.shape[0]])
...
def build_swave_dataloader(tr_folder, eval_folder, sample_rate, b_size):
    train_set = Trainset(json_dir=tr_folder, sample_rate=sample_rate)
    tr_loader = DataLoader(train_set, batch_size=b_size,
                           shuffle=True, num_workers=8)

    eval_set = Trainset(json_dir=eval_folder, sample_rate=sample_rate)
    ev_loader = DataLoader(eval_set, batch_size=b_size,
                           shuffle=True, num_workers=8)
