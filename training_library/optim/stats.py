...
class AverageGradStat(Stat):
    ...
    def update(self, p, state, momentum=0.9, **kwargs):
        state['mom_damp'] = 1-momentum if self.dampening else 1
        state['grad_avg'].mul_(momentum).add_(p.grad.data,
                                              alpha=state['mom_damp'])
        return state
...
class AverageSqrGradStat(Stat):
    ...
    def update(self, p, state, sqr_mom=0.99, **kwargs):
        ...
        state['sqr_avg'].mul_(sqr_mom).addcmul_(p.grad.data,
                                                p.grad.data,
                                                value=state['sqr_damp'])
        return state
