...
class DPMulCat(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.num_layers):
            ...
            if self.training:
                # if we use output += row_output in training the computation
                # will overwrite the comming tensor in the forward step
                # as a result we wont have the original tensor to do the
                # backward and autograd will fail
