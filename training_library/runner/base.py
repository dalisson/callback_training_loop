...
class BaseRunner():
    ...
    def __init__(self, model, data, loss_func, optim, cbs=None):
        ...
        self.training_canceled, self.autograd_debug = False, False
        self.current_batch = None
        self.y_hat, self.x_batch = None, None
        self.y_batch, self.loss = None, None
        ...
        self.iter, self.global_iter = 0, 0
        self.total_iter = 0
        self.in_train = True
    ...
    def all_batches(self, dl):
        ...
        try:
            ...
            for one_batch in dl:
                self.current_batch = one_batch
                self.one_batch(self.current_batch)
    ...
    def begin_fit(self, epochs):
        ...
        self.total_iter = ((len(self.data.train_dl) + len(self.data.valid_dl))
                           * epochs)
    ...
    def fit(self, epochs, additional_cbs=None):
        ...
        try:
            ...
            with torch.autograd.set_detect_anomaly(self.autograd_debug):
    ...
    def save(self, name=None, optimizer=False):
        ...
        sched_states['run_iter'] = self.iter
        if name is None:
            name = 'model_e%s.' % self.epoch
            for metric in self.metrics['eval'].keys():
                name += '{}-{:.3f}.'.format(metric, self.metrics['eval']
                                            [metric][-1])
            name += 'pth'
