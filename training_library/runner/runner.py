...
from ..callbacks.customscheduler import CustomSchedulerCallback
...
from ..callbacks.splitbatch import StandardSplitterCallback
...
STANDARD_CALLBACK_LIST = [CudaCallback(),
                          RecorderCallback(),
                          ProgressbarCallback(),
                          FailOnNanCallback(),
...
class Runner(BaseRunner):
    ...
    def fit_one_cycle(self, n_epochs, max_lr, supress_progress=False,
                      sched_mom=True, **kwargs):
        '''
        One cycle fitting using cosine scheduling.
        '''
        sched = one_cycle_scheduler(lrs=self.lr,
                                    n_param_groups=self.n_param_groups,
                                    max_lr=max_lr, **kwargs)

        lr_scheduler, mom_sched = sched
    ...
    def fit(self, n_epochs, supress_progress=False):
        '''
        Fit with no additional scheduler
        '''
    ...
    def custom_scheduler(self, scheduler, every_iter=False):
        '''
        Add a custom schuder for parameters

        every_iter : scheduler will be called after every iteration
        '''
